...
env.user = 'admin'
env.hosts = ['dev.cgswap.com']
env.root = '/opt/webapps/cgswap.com'
env.proj_root = env.root + '/src/cgswap_com'
...
def push():
    ...
    """
    local('git push', capture=False) # TODO: use an explicit branch here?
    update_proj()
    restart()
...
def sshagent_run(cmd):
    ...
    for h in env.hosts:
        try:
            host, port = h.split(':')
            local('ssh -p %s -A %s@%s "%s"' % (env.user, port, host, cmd))
        except ValueError:
